...
export namespace Config {
  ...
  export const state = Instance.state(async () => {
    ...
    for (const file of ["bootstrap.jsonc", "bootstrap.json"]) {
    ...
    for (const dir of unique(directories)) {
      if (dir.endsWith(".bootstrap") || dir === Flag.OPENCODE_CONFIG_DIR) {
        for (const file of ["bootstrap.jsonc", "bootstrap.json"]) {
  ...
  async function loadCommand(dir: string) {
    ...
    for await (const item of COMMAND_GLOB.scan({
      ...
      const name = (() => {
        const patterns = ["/.bootstrap/command/", "/command/"]
  ...
  async function loadAgent(dir: string) {
    ...
    for await (const item of AGENT_GLOB.scan({
      ...
      const agentFolderPath = item.includes("/.bootstrap/agent/")
        ? item.split("/.bootstrap/agent/")[1]
        : item.includes("/agent/")
          ? item.split("/agent/")[1]
          : agentName + ".md"
  ...
  export const global = lazy(async () => {
    let result: Info = pipe(
      ...
      mergeDeep(await loadFile(path.join(Global.Path.config, "bootstrap.json"))),
      mergeDeep(await loadFile(path.join(Global.Path.config, "bootstrap.jsonc"))),
    )
