...
import { BuildTool, BuildConfig } from "../core/types.js";
 ...
 */
async function detectBuildToolImpl(
  projectPath: string
): Promise<BuildConfig> {
...
async function parseMavenConfig(projectPath: string): Promise<BuildConfig> {
  ...
  try {
    ...
    const isKotlin =
      pomContent.includes("<kotlin.version>") ||
 ...
 */
export async function parseGradleDependencies(
