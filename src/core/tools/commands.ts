...
import { ToolDefinition, RunCommandInput } from "../types.js";
...
const installJavaWithHomebrew = async (): Promise<{
  success: boolean;
  javaHome?: string;
}> => {
  console.log(
  ...
  return new Promise((resolve) => {
    ...
    proc.on("close", (exitCode) => {
      if (exitCode === 0) {
        ...
        console.log(
          `${colors.green}Java installed at ${javaHome}${colors.reset}`
        );
...
const runCommandDefinition: ToolDefinition = {
  name: "run_command",
  description:
    "Execute a shell command with optional working directory and timeout. Provides stdout/stderr and handles timeouts.",
  parameters: {
    type: "object",
    properties: {
      command: {
