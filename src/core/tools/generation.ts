...
const generateTestsDefinition: ToolDefinition = {
  name: "generate_tests",
  description:
    "Analyze a source file and suggest test cases for it. Provide the file path and language. Optionally include coverage data to target gaps.",
  parameters: {
    type: "object",
    properties: {
      file_path: {
    ...
    try {
      ...
      if (input.language === "python") {
        ...
        if (isSpringBoot && componentType) {
          ...
          const testFilePath = resolveTestPath(
            input.file_path,
            buildConfig,
            language
          );
...
const analyzeCoverageGapsDefinition: ToolDefinition = {
  ...
  description:
    "Identify files with low coverage and suggest areas to improve. Provide language and optional minimum coverage threshold.",
  parameters: {
    type: "object",
    properties: {
      language: {
