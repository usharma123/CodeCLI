...
import type { GetDiagnosticsInput, LSPStatusInput, DiagnosticSeverity } from "../lsp/types.js";

const getDiagnosticsDefinition: ToolDefinition = {
  name: "get_diagnostics",
  description:
    "Get code diagnostics (errors, warnings) from language servers. " +
  ...
  parameters: {
    ...
    properties: {
      file: {
        type: "string",
        description:
          "Specific file path to get diagnostics for. " +
  ...
  function: async (input: GetDiagnosticsInput) => {
    ...
    return diagnosticsManager.formatForAgent({
      ...
      severityFilter: input.severity_filter as DiagnosticSeverity[],
      limit: input.limit || 50,
    });
  },
};

const lspStatusDefinition: ToolDefinition = {
  name: "lsp_status",
  description:
    "Check the status of language server processes. " +
  ...
  function: async (_input: LSPStatusInput) => {
    const lspManager = getLSPClientManager();
