import { ToolDefinition, DelegateToAgentInput } from "../types.js";
...
Example: Instead of reading multiple files yourself, delegate to filesystem agent.`,
  parameters: {
    ...
    properties: {
      ...
      context: {
        type: "object",
        description:
          "Additional context for the task (file paths, parameters, etc.)",
      },
      priority: {
        type: "string",
  ...
  function: async (input: DelegateToAgentInput): Promise<string> => {
    // Check if sub-agents are enabled
    if (!isSubAgentsEnabled()) {
      return JSON.stringify({
        error:
          "Sub-agent delegation is currently disabled. Set ENABLE_SUB_AGENTS=true to enable.",
        suggestion:
          "Proceeding with direct tool execution instead of delegation.",
      });
    }
    ...
    try {
      ...
      return JSON.stringify({
        ...
        suggestion:
          "Delegation failed - check if agent is properly initialized",
      });
    }
  },
};
 ...
 */
export const delegationTools: ToolDefinition[] = [delegateToAgentTool];
