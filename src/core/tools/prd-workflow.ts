...
const processPRDWithTasksDefinition: ToolDefinition = {
  name: "process_prd_with_tasks",
  description:
    "Complete PRD processing workflow: parse → extract tasks → approve → execute. This is the main tool for PRD/PDF ingestion with automatic task creation and kickoff.",
  parameters: {
    type: "object",
    properties: {
      prd_file: {
  ...
  function: async (input: ProcessPRDWithTasksInput) => {
    try {
      ...
      const extractTasksTool = prdTaskExtractionTools.find(
        (tool) => tool.name === "extract_tasks_from_prd"
      );
      ...
      const taskPlan: ExtractedTaskPlan = JSON.parse(extractionResult);
...
Please review the PRD format and try again.`;
      ...
      if (autoGenerateTests && taskPlan.testTasks.length > 0) {
        ...
        const generateTestsTool = prdTestingTools.find(
          (tool) => tool.name === "generate_tests_from_prd"
        );
        ...
        if (generateTestsTool) {
          try {
            ...
            console.log(
              `${colors.green}✓ Test templates generated for all features${colors.reset}\n`
            );
          } catch (err) {
            console.log(
              `${colors.yellow}⚠ Test generation failed: ${err}. Continuing with task extraction.${colors.reset}\n`
            );
          }
        }
      }
      ...
      const approved = await confirmAction(
        "Approve and begin execution of these tasks?"
      );
