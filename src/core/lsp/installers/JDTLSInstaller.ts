...
import type { ILSPInstaller, LSPLanguage } from "../types.js";
...
const VERSION_CHECKSUMS: Record<string, string> = {
...
export class JDTLSInstaller implements ILSPInstaller {
  language: LSPLanguage = "java";

  private readonly jdtLsDir: string;
  ...
  async install(): Promise<void> {
    ...
    try {
      ...
      throw new Error(
        "Java runtime not found. Please install Java 17+ to use Java LSP support.\n" +
    ...
    try {
      ...
      if (expectedChecksum) {
        ...
        console.warn(
          `Warning: No checksum available for version ${versionKey}. Skipping integrity verification.`
        );
      }
      ...
      throw new Error(
        `Failed to install JDT LS: ${error instanceof Error ? error.message : String(error)}`
      );
    } finally {
   ...
   */
  async getServerCommand(): Promise<string[]> {
    const launcherJar = this.findLauncherJar();
    if (!launcherJar) {
