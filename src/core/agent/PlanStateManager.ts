...
import type { Plan, PlanState, TodoItem, IPlanStateManager } from "./types.js";

export class PlanStateManager implements IPlanStateManager {
   ...
   */
  private convertToActiveForm(task: string): string {
    ...
    return task.replace(
      /^(Add|Create|Implement|Update|Fix|Remove|Write|Build|Set up|Setup|Configure|Install|Test|Refactor|Delete|Move|Rename|Extract|Merge|Split)/i,
      (match) => {
