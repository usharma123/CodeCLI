...
import { useDialog, type DialogConfig } from "../context/dialog.js";
...
function ConfirmDialog(props: { config: DialogConfig }) {
  const { theme } = useTheme();
  const [selected, setSelected] = createSignal<"yes" | "no">("yes");
    ...
    <box flexDirection="column">
      ...
      <box flexDirection="row" gap={2} justifyContent="center">
        <box
          borderStyle="single"
          borderColor={
            selected() === "yes" ? theme().colors.success : theme().colors.border
          }
          paddingX={2}
        >
          <text
            bold={selected() === "yes"}
            color={selected() === "yes" ? theme().colors.success : undefined}
          >
            Yes (y)
          </text>
        </box>
        <box
          borderStyle="single"
          borderColor={
            selected() === "no" ? theme().colors.error : theme().colors.border
          }
          paddingX={2}
        >
          <text
            bold={selected() === "no"}
            color={selected() === "no" ? theme().colors.error : undefined}
          >
            No (n)
          </text>
        </box>
...
function AlertDialog(props: { config: DialogConfig }) {
  const { theme } = useTheme();
...
function PromptDialog(props: { config: DialogConfig }) {
  const { theme } = useTheme();
...
function SelectDialog(props: { config: DialogConfig }) {
  const { theme } = useTheme();
