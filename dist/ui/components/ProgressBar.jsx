...
export function ProgressBar({ progress, width, color = "cyan", size = "standard", showPercentage = true, showLabel = false, label, animated = true, }) {
    ...
    return (<Box>
      {showLabel && label && (<Box marginRight={1}>
          <Text dimColor>{label}</Text>
        </Box>)}
      <Text color={color}>{filled}{partial}</Text>
      <Text dimColor>{empty}</Text>
      {showPercentage && (<Text dimColor> {Math.round(clampedProgress)}%</Text>)}
    </Box>);
}
        ...
        {stages.map((stage, i) => {
            ...
            const textColor = stage.status === "completed"
                ...
                : stage.status === "current"
                    ? color
                    : "gray";
            return (<Box key={i} marginRight={2}>
              <Text color={textColor}>{icon} </Text>
              <Text color={textColor} dimColor={stage.status === "pending"}>
                {stage.label}
              </Text>
            </Box>);
        })}
