...
export function Panel({ children, title, titleIcon, variant = "default", color = "cyan", footer, collapsible = false, defaultCollapsed = false, paddingX = 1, paddingY = 0, marginBottom = 1, width, }) {
    ...
    return (<Box flexDirection="column" borderStyle={showBorder ? borderStyle : undefined} borderColor={showBorder ? borderColor : undefined} paddingX={paddingX} paddingY={paddingY} marginBottom={marginBottom} width={width}>
      {/* Header */}
      {title && (<Box marginBottom={collapsed ? 0 : 1}>
          <Text color={color} bold>
            {titleIcon && `${titleIcon} `}
            {title}
          </Text>
          {collapsible && (<Text color="gray">
              {" "}
      ...
      {/* Footer */}
      {!collapsed && footer && (<Box marginTop={1} borderStyle="single" borderColor="gray" borderTop borderBottom={false} borderLeft={false} borderRight={false}>
          {footer}
        </Box>)}
    </Box>);
}
...
export function InfoPanel({ children, title }) {
    return (<Panel title={title} titleIcon={icons.info} color="cyan" variant="subtle">
      {children}
    </Panel>);
}
export function SuccessPanel({ children, title }) {
    return (<Panel title={title} titleIcon={icons.success} color="green" variant="default">
      {children}
    </Panel>);
}
export function WarningPanel({ children, title }) {
    return (<Panel title={title} titleIcon={icons.warning} color="yellow" variant="default">
      {children}
    </Panel>);
}
export function ErrorPanel({ children, title }) {
    return (<Panel title={title} titleIcon={icons.error} color="red" variant="accent">
      {children}
    </Panel>);
}
