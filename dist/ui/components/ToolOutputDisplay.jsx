...
export function ToolOutputDisplay({ expandedOutputId }) {
    ...
    return (<Box flexDirection="column" borderStyle="single" borderColor="blue" paddingX={1} marginBottom={1}>
      {/* Header */}
      <Box marginBottom={1}>
...
function OutputContent({ content }) {
    ...
    if (looksLikeJson) {
        try {
            ...
            return (<Box borderStyle="single" borderColor="gray" paddingX={1}>
    ...
    if (looksLikeFilePaths) {
        ...
        {lines.slice(0, 20).map((line, i) => (<Box key={i}>
            <Text dimColor>{icons.bullet} </Text>
            <Text>{line.trim()}</Text>
          </Box>))}
        {lines.length > 20 && (<Text dimColor>+{lines.length - 20} more</Text>)}
      </Box>);
    }
    ...
    return (<Box flexDirection="column" borderStyle="single" borderColor="gray" paddingX={1}>
      {lines.slice(0, 50).map((line, i) => (<Box key={i}>
          {showLineNumbers && (<Box minWidth={4}>
              <Text dimColor>{i + 1}</Text>
            </Box>)}
          <Text wrap="wrap">{line}</Text>
        </Box>))}
      {lines.length > 50 && (<Text dimColor>+{lines.length - 50} lines</Text>)}
    </Box>);
}
// Compact preview
