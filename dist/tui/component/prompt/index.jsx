...
export function Prompt(props) {
    ...
    return (<box flexDirection="column">
      ...
      <Show when={showAutocomplete()}>
        <Autocomplete query={value().slice(1)} onSelect={(cmd) => {
            setValue(`/${cmd} `);
            setShowAutocomplete(false);
      ...
      {/* Input box */}
      <box borderStyle="single" borderColor={borderColor()} paddingX={1} flexDirection="row">
        {/* Mode icon */}
        ...
        {/* Input field */}
        <input value={value()} onChange={(e) => handleChange(e.target.value)} onKeyDown={(e) => handleKeyDown(e.key, e.ctrlKey)} placeholder={state.isProcessing
            ? "Processing..."
            : mode() === "command"
                ? "Type a command..."
                : mode() === "file"
                    ...
                    : mode() === "shell"
                        ? "Enter shell command..."
                        : "Type a message..."} disabled={isSubmitting()} focus flexGrow={1}/>
