...
export function MultiSpinner(props) {
    const { theme } = useTheme();
    return (<box flexDirection="column">
      {props.stages.map((stage, index) => {
            ...
            return (<box flexDirection="row" gap={1}>
            {isComplete && (<text color={theme().colors.success}>{theme().icons.success}</text>)}
            {isCurrent && <Spinner color={props.color}/>}
            {isPending && (<text color={theme().colors.muted}>{theme().icons.pending}</text>)}
            <text color={isPending ? theme().colors.muted : undefined} strikethrough={isComplete}>
              {stage}
            </text>
          </box>);
        })}
