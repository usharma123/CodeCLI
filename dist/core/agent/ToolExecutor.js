...
export class ToolExecutor {
    ...
    async executeToolCall(toolCall) {
        ...
        const parseResult = this.jsonHelper.parseAndFixJson(toolCall.function.arguments, functionName, true);
        // Handle parse failure
        if (!parseResult.success) {
            this.jsonHelper.logParseErrorContext(toolCall.function.arguments, parseResult.error || "Unknown error");
            return {
                tool_call_id: toolCall.id,
                role: "tool",
                content: this.jsonHelper.generateParseErrorMessage(functionName, toolCall.function.arguments, parseResult.error || "Unknown error"),
            };
        }
        ...
        if (parseResult.wasTruncated && functionName === "write_file") {
            ...
            return {
                tool_call_id: toolCall.id,
                role: "tool",
                content: this.jsonHelper.generateTruncationErrorMessage(functionArgs, parseResult.truncationInfo || ""),
            };
        }
...
- Permission issues`,
     ...
     */
    validateWriteFileArgs(functionArgs, toolCallId) {
