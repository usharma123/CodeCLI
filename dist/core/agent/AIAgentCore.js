...
${colors.reset}`);
    ...
    async processMessage() {
        try {
            ...
            const { message, elapsedSeconds, streamedContent, reasoningDetails } = await this.completionService.createCompletion({
                model: this.model,
    ...
    async handleToolCalls(toolCalls) {
        ...
        for (const result of toolCallResults) {
            this.messageHistory.addMessage(result);
        }
        try {
            // Get follow-up response after tool execution
            emitStatus({ phase: "thinking", message: "Thinking…" });
            const { message: followUpMessage, streamedContent, reasoningDetails } = await this.completionService.createCompletion({
                model: this.model,
                messages: this.messageHistory.getMessages(),
                tools: this.toolExecutor.getOpenAITools(),
                tool_choice: "auto",
     ...
     */
    async retryWithToolEnforcement(originalStreamed) {
        console.log(`  ${colors.gray}└ Retrying with tool enforcement...${colors.reset}`);
        emitStatus({ phase: "thinking", message: "Thinking (retry)…" });
        const { message: fallbackMessage, streamedContent, reasoningDetails } = await this.completionService.createCompletion({
            model: this.model,
            messages: this.messageHistory.getMessages(),
            tools: this.toolExecutor.getOpenAITools(),
            tool_choice: "auto",
     ...
     */
    displayReasoning(reasoningDetails) {
     ...
     */
    async handleProcessMessageError(error) {
