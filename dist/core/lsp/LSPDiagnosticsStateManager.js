...
export class LSPDiagnosticsStateManager {
    state;
    constructor() {
        this.state = {
            diagnostics: new Map(),
     ...
     */
    updateServerStatus(language, status) {
    ...
    formatForAgent(options = {}) {
        ...
        if (options.severityFilter && options.severityFilter.length > 0) {
            diagnostics = diagnostics.filter((d) => options.severityFilter.includes(d.severity));
        }
