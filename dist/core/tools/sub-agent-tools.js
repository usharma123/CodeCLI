...
This tool consumes significant context and should only be used when the scope genuinely requires exploration.`,
    parameters: {
        ...
        properties: {
            pattern: {
                type: "string",
                description: "Glob pattern, search term, or description of what to find (e.g., '*.ts', 'authentication logic', 'test files')",
            },
            focus: {
                type: "string",
                description: "What aspect to focus on (e.g., 'find all React components', 'understand authentication flow', 'locate configuration files')",
            },
            depth: {
                type: "string",
                enum: ["quick", "thorough"],
                description: "Exploration depth: 'quick' for fast overview, 'thorough' for comprehensive analysis",
            },
        },
    ...
    function: async (input) => {
        try {
            ...
            const task = createAgentTask("filesystem", `Explore codebase: pattern="${input.pattern}". Focus: ${input.focus}. Depth: ${input.depth || "quick"}.`, {
 ...
 */
const analyzeCodeImplementationTool = {
...
This tool consumes significant context and is designed for architectural understanding, not line-by-line code reading.`,
    parameters: {
        ...
        properties: {
            files: {
                type: "array",
                items: { type: "string" },
                description: "List of file paths to analyze (can include glob patterns)",
            },
            focus: {
                type: "string",
                description: "What to focus on in the analysis (e.g., 'authentication architecture', 'data flow', 'API design patterns')",
            },
            analysis_type: {
                type: "string",
                enum: ["architecture", "requirements", "review"],
                description: "Type of analysis: 'architecture' for system design, 'requirements' for PRD parsing, 'review' for code quality",
            },
        },
    ...
    function: async (input) => {
        try {
            ...
            const task = createAgentTask("analysis", `Analyze ${input.analysis_type}: ${input.files.length} file(s). Focus: ${input.focus}.`, {
                files: input.files,
                focus: input.focus,
...
This tool is optimized for parallel processing of many files and should only be used when the operation genuinely requires bulk handling.`,
    ...
    function: async (input) => {
        try {
            ...
            // Create structured task for FileSystemAgent
            const task = createAgentTask("filesystem", `Bulk ${input.operation}: ${input.files.length} file(s).${input.pattern ? ` Pattern: ${input.pattern}` : ""}`, {
