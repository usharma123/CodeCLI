    ...
    function: async (input) => {
        try {
            ...
            for (const match of headerMatches) {
                ...
                if (!title.toLowerCase().match(/^(introduction|overview|background|conclusion|appendix|table of contents|homework|assignment|evaluation|testing|what to submit)/)) {
                    requirements.push({
            ...
            for (const match of plainHeaderMatches) {
                const title = match[1].trim();
                if (!title.toLowerCase().match(/^(introduction|overview|background|conclusion|appendix|table of contents|homework|assignment|evaluation|testing|what to submit|the assignment|suggested approach)/) &&
            ...
            for (const req of requirements) {
                const isDuplicate = uniqueRequirements.some((existing) => existing.title.toLowerCase() === req.title.toLowerCase() ||
            ...
            if (granularity === "high-level" && uniqueRequirements.length > 10) {
                ...
                finalRequirements = Array.from(grouped.entries()).map(([title, reqs]) => ({
                    ...
                    type: "feature",
                    priority: reqs.some(r => r.priority === "high") ? "high" : "normal",
                    requiresTest: true,
                }));
            }
