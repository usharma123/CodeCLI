...
const runTestsDefinition = {
    ...
    function: async (input) => {
        try {
            ...
            const { stdout, stderr, exitCode, timedOut } = await runShellCommand(command, workingDir, timeoutMs, process.env, false // Don't stream - prevents UI corruption
            );
            let result = `Test Execution Results\n`;
            result += `======================\n\n`;
            if (resultHeader)
            ...
            if (language === "java" || language === "all") {
                ...
                if (surefireDir && fs.existsSync(surefireDir)) {
                    const reports = fs.readdirSync(surefireDir)
                        .filter((file) => file.endsWith(".xml") && file.startsWith("TEST-"));
                    if (reports.length > 0) {
 ...
 */
const findSurefireReports = (baseDir) => {
    const possiblePaths = [
 ...
 */
const parseSurefireReports = (surefireDir) => {
    let total = 0;
    let failed = 0;
    try {
        const reports = fs.readdirSync(surefireDir)
