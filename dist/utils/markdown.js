...
const candidatePathRegex = /\b[A-Za-z0-9._-]+(?:[\\/][A-Za-z0-9._-]+)*\.[A-Za-z]{1,8}\b/g;
...
export function renderMarkdownToAnsi(markdown) {
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (headingMatch) {
            ...
            output.push(`${headingColor}${colors.bold}${renderInline(text)}${colors.reset}`);
            continue;
        }
        ...
        if (blockquoteMatch) {
            output.push(`${colors.gray}│ ${colors.reset}${renderInline(blockquoteMatch[1])}`);
            continue;
        }
        ...
        if (orderedMatch) {
            ...
            output.push(`${indent}${colors.yellow}${num}.${colors.reset} ${renderInline(orderedMatch[3])}`);
            continue;
        }
        const unorderedMatch = line.match(/^(\s*)[-+*]\s+(.*)$/);
        if (unorderedMatch) {
            const indent = unorderedMatch[1];
            output.push(`${indent}${colors.yellow}•${colors.reset} ${renderInline(unorderedMatch[2])}`);
            continue;
        }
...
function renderTokens(tokens, activeStyle) {
    let output = "";
...
function parseInline(text) {
    ...
    while (i < text.length) {
        ...
        if (ch === "*" || ch === "_") {
            const prev = text[i - 1];
            const next = text[i + 1];
            const looksLikeWordBoundary = !isWordChar(prev) && !isWordChar(next) && next !== " ";
...
function renderTableLine(line, isHeader) {
    ...
    const cells = trimmed
        ...
        .split("|")
        .map((c) => c.trim());
    const renderedCells = cells.map((cell) => isHeader
