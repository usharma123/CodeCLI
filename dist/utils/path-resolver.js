 ...
 */
export function resolveTestPath(sourcePath, buildConfig, language) {
    // 1. Verify source path contains expected source directory
    if (!sourcePath.includes(buildConfig.mainSourceDir)) {
        console.warn(`Path ${sourcePath} doesn't match expected source dir ${buildConfig.mainSourceDir}`);
    ...
    const testPath = sourcePath.substring(0, lastIndex) +
    ...
    const testFileName = testPath.replace(new RegExp(`\\.${extension}$`), `Test.${extension}`);
    return testFileName;
}
/**
 ...
 */
export function isValidSourcePath(sourcePath, buildConfig) {
 ...
 */
export function getPackageRelativePath(sourcePath, buildConfig) {
    ...
    const afterSourceDir = sourcePath.substring(index + sourceDir.length + 1 // +1 for the path separator
    );
