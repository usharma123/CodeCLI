 ...
 */
export async function isSpringBootProject(projectPath, buildConfig) {
    try {
 ...
 */
async function isSpringBootMavenProject(projectPath) {
    try {
 ...
 */
async function isSpringBootGradleProject(projectPath) {
    try {
 ...
 */
export async function isReactiveProject(projectPath, buildConfig) {
    try {
        if (!buildConfig || buildConfig.tool === "maven") {
            const pomPath = path.join(projectPath, "pom.xml");
            if (fs.existsSync(pomPath)) {
                const pomContent = fs.readFileSync(pomPath, "utf-8");
                return (pomContent.includes("spring-boot-starter-webflux") ||
        ...
        if (!buildConfig || buildConfig.tool === "gradle") {
            ...
            for (const gradleFile of gradleFiles) {
                if (fs.existsSync(gradleFile)) {
                    ...
                    if (gradleContent.includes("spring-boot-starter-webflux") ||
 ...
 */
export function detectRepositoryType(sourceCode) {
    // Check for reactive repositories first
    if (/extends\s+ReactiveCrudRepository\b/.test(sourceCode) ||
